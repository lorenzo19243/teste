...
$hoje = date("Y-m-d");
$consulta = 'SELECT * FROM recolhas WHERE DATE(agendado) = CURDATE() AND STATUS="ANDAMENTO"  
union 
(SELECT * FROM recolhas WHERE DATE(agendado) = CURDATE() AND STATUS="REMARCAR" )
union 
(SELECT * FROM recolhas WHERE DATE(agendado) = CURDATE() AND STATUS="DROP" )
union
(SELECT * FROM recolhas WHERE DATE(agendado) = CURDATE() AND STATUS="A LANÇAR")
';
...
</svg></button> <a class="btn btn-warning position-relative btn-sm"  href="listar_recolhas_hoje.php">
HOJE <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-list-check" viewBox="0 0 16 16">
  <path fill-rule="evenodd" d="M5 11.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5m0-4a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5m0-4a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5M3.854 2.146a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 1 1 .708-.708L2 3.293l1.146-1.147a.5.5 0 0 1 .708 0m0 4a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 1 1 .708-.708L2 7.293l1.146-1.147a.5.5 0 0 1 .708 0m0 4a.5.5 0 0 1 0 .708l-1.5 1.5a.5.5 0 0 1-.708 0l-.5-.5a.5.5 0 0 1 .708-.708l.146.147 1.146-1.147a.5.5 0 0 1 .708 0"/>
</svg></a></li>
        </ul>
...
<div class="row">
          ...
          <div class="col-md-4 "><div class="card bg-warning-subtle text-warning-emphasis">

  <div class="card-body">
    <h5 class="card-title">OPS!</h5>
    <p class="card-text">NENHUM AGENDAMENTO NO MOMENTO!!!.</p>
  </div>
</div>
...
echo ' <table class="table table-striped table-sm" style="font-size: 14px;">
  ...
  <path d="M6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m-5 6s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zM11 3.5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5m.5 2.5a.5.5 0 0 0 0 1h4a.5.5 0 0 0 0-1zm2 3a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1zm0 3a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1z"/>
            ...
            <th scope="col" style="border:none" width="20%"><div class="fs-6 badge d-flex text-light-emphasis bg-light-subtle"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-geo-alt-fill" viewBox="0 0 16 16">
...
?></td>
                <td style="text-align: center; vertical-align: middle;"><h5><span class="badge text-bg-secondary border-0 rounded-pill p-1"><?php echo date('d/m/y',  strtotime($agendado));?></span><br><span class="badge text-bg-warning text-light rounded-pill p-1">ÁS <?php echo date('H:i',  strtotime($agendado));?></span></h5></td>
...
*TIPO:* RECOLHER TODOS OS EQUIPAMENTOS     
    ...
    $(document).ready(function() {
        ...
        $('#insert_form').on('submit', function(event) {
            ...
            if (!nome.val().match(/[A-Za-z\d]/)) {
                ...
                $.post("processa_cad_recolhas.php", dados, function(retorna) {
                    if (retorna) {
                        //Limpar os campo
                        $('#insert_form')[0].reset();
                        ...
                        setInterval(function() {
                            var redirecionar = "listar_recolhas.php";
...
<div class="col-md-12 col-sm-12">
          ...
          <label for="recipient-urgente" class="col-form-label">ROTEADOR</label>
                                        <select class="form-select" value="" name="roteador_entrada" id="roteador_entrada">
                                <option value="" selected required>Selecione...</option>
                                <?php
$consulta = "SELECT * FROM hb_estoque WHERE tipo='roteador'";
$resultado = mysqli_query($conn, $consulta);
date_default_timezone_set('America/Sao_Paulo');
// CRIA UMA VARIAVEL E ARMAZENA A HORA ATUAL DO FUSO-HORÀRIO DEFINIDO (BRASÍLIA)
$dataLocal = date('d/m/Y H:i:s', time());
...
?>
        ...
        <input type="text" class="form-control" name="roteador_quantidade_entrada" value="">
      </div>
          <div class="col-md-3 col-sm-12">
          <label for="recipient-urgente" class="col-form-label">ONU</label>
                                        <select class="form-select" value="" name="onu_entrada" id="onu_entrada">
                                <option value="" selected required>Selecione...</option>
                                <?php
...
?>
                   ...
                   <div class="col-md-1">
        <label for="exampleFormControlInput1" name="email"  class="col-form-label">QTD:</label>
        <input type="text" class="form-control" name="onu_quantidade_entrada" value="">
      </div>
          <div class="col-md-3 col-sm-12">
          <label for="recipient-urgente" class="col-form-label">TIMER</label>
                                        <select class="form-select" value="" name="timer_entrada" id="timer_entrada">
                                <option value="" selected required>Selecione...</option>
                                <?php
...
?>
                        ...
                        ?>  
                             ...
                             <div class="col-md-1">
        <label for="exampleFormControlInput1" name="email"  class="col-form-label">QTD:</label>
        <input type="text" class="form-control" name="timer_quantidade_entrada" value="">
      </div>
 <div class="col-md-2 col-sm-12">
            <label for="recipient-rua" class="col-form-label">CABO DROP:</label>
            <input type="text" name="cabo_drop" id="cabo_drop" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
            <label for="recipient-prensa" class="col-form-label">CABO UTP:</label>
            <input type="text" name="cabo_utp" id="recipient-prensa" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
...
function processa_rec() {
    ...
    xhttp.open("GET", "processa_recolhas.php?logins="+logins);
    xhttp.send();    
}      
</script>
...
$consulta = "SELECT * FROM recolhas WHERE agendado='$hoje' AND STATUS ='A LANÇAR'";
$resultado = mysqli_query($conn, $consulta);			
...
echo '<div class="alert alert-danger" role="alert">
  Ei! temos recolhas para hoje, bora cuidar?
</div> ';
}
...
?>
    ...
    <div class="d-flex text-body-secondary pt-3">
      ...
      <p class="pb-3 mb-0 small lh-sm border-bottom">
        <strong class="d-block text-gray-dark"> <?php echo $nome ?></strong>
        NO BAIRRO: <?php echo $bairro ?>, PLANO: <?php echo $plano ?>.
      </p>
    </div>
   
    <?php } ?>
    <div class="alert alert-danger" role="alert">
  TEMOS <?php $clientes = mysqli_query($conn,'SELECT * FROM recolhas WHERE  STATUS="A LANÇAR" ORDER BY nome DESC;');
				$row_cnt = mysqli_num_rows($clientes);

printf("%d", $row_cnt); ?> RECOLHAS EM ATRASO PARA LANÇAR!
</div> 
            </div>
            <div class="modal-footer">
