...
$altera_cliente = "UPDATE mudanca SET tecnico='$tecnico', veiculo='$veiculo', alterado_por='$alterado_por', ultima_alteracao='$ultima_alteracao', status='$status',  rede='$rede', redesenha='$redesenha', conclusao='$conclusao', sinal='$sinal',  agendado='$agendado', conector='$conector', buxa_parafuso='$buxa_parafuso', esticador='$esticador', remoto='$remoto', repetidor='$repetidor', concluido='$concluido', data_fim='$data_fim', tempo='$tempo'  WHERE id_cliente='$id_cliente'";
$log_categoria = "INSERT INTO hb_os_logs set log='ALTEROU O STATUS DA MUDANÇA DE $nome PARA $status', data='$ultima_alteracao', utilizador='$alterado_por'";
$resposta = mysqli_query($conn, $altera_cliente);
$resposta .= mysqli_query($conn, $log_categoria);


if($drop_solto == 'NAO' and $cabo_drop != '' and $concluido == 'SIM'){ 
    
$sql1="UPDATE mudanca set cabo_drop ='$qtd_formatada_drop' WHERE id_cliente='$id_cliente'";
...
if($drop_solto == 'SIM' and $cabo_drop != '' and $concluido == 'SIM'){ 
    
$sql5="UPDATE mudanca set drop_solto ='$qtd_formatada_drop' WHERE id_cliente='$id_cliente'";
$resultado5 = mysqli_query($conn, $sql5);

$log_estoque3 = "INSERT INTO hb_logs_estoque_geral set log='USADO $qtd_formatada_drop METROS DE CABO DROP SOLTO', motivo='REPARO DE $nome', data='$ultima_alteracao', utilizador='$alterado_por'";
$resposta_estoque3 = mysqli_query($conn, $log_estoque2);
  }  
