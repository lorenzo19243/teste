...
seguranca_adm();
$consulta = 'SELECT * FROM cameras WHERE  STATUS="ANDAMENTO"   
union 
(SELECT * FROM cameras WHERE  STATUS="REMARCAR" )
union
(SELECT * FROM cameras WHERE  STATUS="A LANÇAR" ORDER BY data_cadastro DESC) 
...
$interval = $data1->diff($data2);
    ?>
      ...
      <tr >
        <td style="text-align: center; vertical-align: middle;"><?php echo $id_cameras ?></td>
        <td ><span style="font-weight: bold;"><?php echo $nome ?> </span><br>BAIRRO - <?php echo $bairro ?> <a href="https://app.xgstelecom.com.br/os/cameras.php?id=<?php echo $linha['id_cameras']; ?>"  target="_blank" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-globe-americas" viewBox="0 0 16 16">
  <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0M2.04 4.326c.325 1.329 2.532 2.54 3.717 3.19.48.263.793.434.743.484q-.121.12-.242.234c-.416.396-.787.749-.758 1.266.035.634.618.824 1.214 1.017.577.188 1.168.38 1.286.983.082.417-.075.988-.22 1.52-.215.782-.406 1.48.22 1.48 1.5-.5 3.798-3.186 4-5 .138-1.243-2-2-3.5-2.5-.478-.16-.755.081-.99.284-.172.15-.322.279-.51.216-.445-.148-2.5-2-1.5-2.5.78-.39.952-.171 1.227.182.078.099.163.208.273.318.609.304.662-.132.723-.633.039-.322.081-.671.277-.867.434-.434 1.265-.791 2.028-1.12.712-.306 1.365-.587 1.579-.88A7 7 0 1 1 2.04 4.327Z"/>
</svg></a></td>
        <td><?php echo $situacao ?> </br><?php
$observacao = $observacao;
$id_cameras = $id_cameras;

if ($observacao >= '0') {
    $os = "<button type='button' class='badge rounded-pill text-bg-warning text-light border-0' data-toggle='modal' data-target='#modal$id_cameras'>
OBS!
</button>";
} else {
    $os = "";
...
<strong>ALTERADO POR:</strong> <?php echo $alterado_por ?> - <?php echo $ultima_alteracao = date('d/m/Y H:i:s',  strtotime($ultima_alteracao));?> <br>
        ...
        <td style="vertical-align: middle;"><a class="accordion-button" href="#" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#editarCliente" data-whatever="<?php echo $linha['id_cameras']; ?>" 
...
*ORDEM DE SERVIÇO Nº <?php echo $linha['id_cameras']; ?>*
`<?php echo $situacao ?>`
...
*END:* <?php echo $linha['rua']; ?>, <?php echo $linha['numero']; ?> - <?php echo $linha['bairro']; ?>  
...
<div class="modal fade" id="cadCliente" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header ">
        <h5 class="modal-title" id="exampleModalLabel">AGENDAR CHAMADO CAMERAS!</h5>
        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
      </div>
...
<?php
        ...
        <div class="row">
            <div class="col-md-3 col-sm-12">
             <label for="recipient-urgente" class="col-form-label">CAMERA EXT</label>
                                        <select class="form-select" value="" name="camext_saida" id="camext_saida">
                                <option value="" selected required>Selecione...</option>
                                <?php
$consulta = "SELECT * FROM hb_estoque WHERE tipo='camera' ORDER BY produto";
$resultado = mysqli_query($conn, $consulta);
date_default_timezone_set('America/Sao_Paulo');
// CRIA UMA VARIAVEL E ARMAZENA A HORA ATUAL DO FUSO-HORÀRIO DEFINIDO (BRASÍLIA)
$dataLocal = date('d/m/Y H:i:s', time());
...
?>
        ...
        <input type="text" class="form-control" value="0" name="camext_qtd_saida" value="">
      </div>
      <div class="col-md-3 col-sm-12">
             <label for="recipient-urgente" class="col-form-label">CAMERA INT</label>
                                        <select class="form-select" value="" name="camint_saida" id="camint_saida">
                                <option value="" selected required>Selecione...</option>
                                <?php
$consulta = "SELECT * FROM hb_estoque WHERE tipo='camera' ORDER BY produto";
$resultado = mysqli_query($conn, $consulta);
...
?>
         ...
         <div class="col-md-1">
        <label for="exampleFormControlInput1" name="cameraint"  class="col-form-label">QTD:</label>
        <input type="text" class="form-control" value="0" name="camint_qtd_saida" value="">
      </div>
      <div class="col-md-3 col-sm-12">
...
?>
          ...
          <div class="col-md-1">
        <label for="exampleFormControlInput1" name="email"  class="col-form-label">QTD:</label>
        <input type="text" class="form-control" value="0" name="onu_quantidade_saida" value="">
      </div>
      <div class="col-md-2 col-sm-12">
            <label for="recipient-rua" class="col-form-label">CABO REDE:</label>
            <input type="text" name="cabo_rede" value="0" id="cabo_drop" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
            <label for="recipient-repetidor" class="col-form-label">CABO SOLTO:</label>
            <select class="form-select" name="cabo_solto" id="cabo_solto" aria-label=".form-select-lg example">
              <option value="NAO">NAO</option>
              <option value="SIM">SIM</option>
            </select>
          </div> 
           <div class="col-md-2 col-sm-12">
            <label for="recipient-prensa" class="col-form-label">PRENSA FIO:</label>
            <input type="text" name="prensa_fio" value="0" id="recipient-prensa" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
            <label for="recipient-rua" class="col-form-label">CONECTOR:</label>
            <input type="text" name="conector" value="0" id="conector" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
            <label for="recipient-rua" class="col-form-label">ESTICADORES:</label>
            <input type="text" name="esticadores" value="0" id="esticadores" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-2 col-sm-12">
            <label for="recipient-rua" class="col-form-label">BUXA/PARAF.:</label>
            <input type="text" name="bucha_parafuso" value="0" id="bucha_parafuso" maxlength="50" class="form-control -10">
          </div>
         <div class="col-md-1 col-sm-12">
            <label for="recipient-rua" class="col-form-label">P. P4.:</label>
            <input type="text" name="plugp4" value="0" id="plugp4" maxlength="50" class="form-control -10">
          </div>
         <div class="col-md-1 col-sm-12">
            <label for="recipient-rua" class="col-form-label">RJ45.:</label>
            <input type="text" name="rj45" value="0" id="rj45" maxlength="50" class="form-control -10">
          </div>
          <div class="col-md-3 col-sm-12">
                         <label for="recipient-agendado" class="col-form-label">REMARCAR DIA:</label>
                         <input class="w3-input w3-border form-control " value="<?php echo date("Y-m-d") ?>" type="date"  name="agendado" id="recipient-agendado" required>
             </div>        
             <div class="col-md-3 col-sm-12">
            <label for="recipient-sinal" class="col-form-label">SINAL:</label>
            <input type="text" class="form-control" name="sinal" id="recipient-sinal">
          </div>
                     <div class="col-md-2 col-sm-12">
