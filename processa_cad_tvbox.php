...
$agendado = mysqli_real_escape_string($conn, $_POST['agendado']);
    ...
    $link = mysqli_query($conn, "SELECT nome FROM tvbox_chamados WHERE nome = '$nome' AND  STATUS='A LANÇAR'");
...
$altera_cliente = "INSERT INTO tvbox_chamados (nome, situacao, observacao, rua, numero, bairro, data_cadastro, criado_por, status, sinal, pppoe, plano, rede, redesenha, concluido, agendado) 
VALUES ('$nome', '$situacao', '$observacao', '$rua', '$numero', '$bairro', '$data_cadastro', '$criado_por', '$status', '$sinal', '$pppoe', '$plano', '$rede', '$redesenha', '$concluido' , '$agendado')";
$log_categoria = "INSERT INTO hb_os_logs set log='AGENDOU $situacao DE $nome', data='$ultima_alteracao', utilizador='$criado_por'";
